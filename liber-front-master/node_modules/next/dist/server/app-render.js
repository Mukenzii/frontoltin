...
const use = _react.default.experimental_use;
 ...
 */ function useFlightResponse(writable, req, serverComponentManifest, flightResponseRef, nonce) {
    if (flightResponseRef.current) {
        return flightResponseRef.current;
    }
    const [renderStream, forwardStream] = (0, _nodeWebStreamsHelper).readableStreamTee(req);
    flightResponseRef.current = (0, _reactServerDomWebpack).createFromReadableStream(renderStream, {
 ...
 * This is only used for renderToHTML, the Flight response does not need additional wrappers.
 */ function createServerComponentRenderer(ComponentToRender, ComponentMod, { transformStream , serverComponentManifest , serverContexts  }, nonce) {
    ...
    return function ServerComponentWrapper() {
        ...
        const response = useFlightResponse(writable, reqStream, serverComponentManifest, flightResponseRef, nonce);
        return use(response);
    };
}
/**
...
async function renderToHTMLOrFlight(req, res, pathname, query, renderOpts, isPagesDir) {
   ...
   */ const createComponentTree = async ({ createSegmentPath , loaderTree: [segment, parallelRoutes, { layoutOrPagePath , layout , template , error , loading , page  }, ] , parentParams , firstItem , rootLayoutIncluded  })=>{
     ...
     */ const currentParams = // Handle null case where dynamic param is optional
        ...
        const segmentPath1 = createSegmentPath([
            actualSegment
        ]);
        const dataCacheKey = JSON.stringify(segmentPath1);
        let fetcher = null;
